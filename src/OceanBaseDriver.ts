...
export interface OceanBaseDriverConfiguration extends ConnectionConfig {
  readOnly?: boolean,
...
interface OceanBaseConnection extends Connection {
  execute: (options: string | QueryOptions, values?: any) => Promise<any>
}

/**
 * OceanBase driver class.
 */
export class OceanBaseDriver extends BaseDriver implements DriverInterface {
   ...
   */
  public constructor(
    config: OceanBaseDriverConfiguration & {
      dataSource?: string,
      maxPoolSize?: number,
    } = {}
  ) {
  ...
  protected withConnection(fn: (conn: OceanBaseConnection) => Promise<any>) {
    const self = this;
  ...
  public async testConnection() {
    ...
    const conn: OceanBaseConnection = await (<any> this.pool)._factory.create();
  ...
  protected setTimeZone(conn: OceanBaseConnection) {
